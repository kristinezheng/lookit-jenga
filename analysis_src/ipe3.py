...
class Sim:
    ...
    def create_structure(self, blocks):
        ...
        for i in range(len(blocks)):
            x, y, theta = blocks[i]
            if i == len(blocks)-1:
                y += 0.3 #abs(np.random.normal(0,0.5))
    ...
    def monte_carlo_shift(self, scale_pos=(0.1, 0.1), scale_ori=np.pi / 16):
        """
        Perturb the position and yaw of each block in the structure by a Gaussian.
        """
        ids = [i for i in self.bodies]
        i = ids[-1]
        ...
        # Perturb the position in the x and y axes
        perturbation_pos = np.pad(np.random.normal(0., scale=scale_pos, size=(2,)), (0, 1))
        perturbation_pos[1] = abs(np.random.normal(0,0.5))
        # Perturb the orientation in the z axis
        # perturbation_ori = np.pad(np.random.normal(0., scale=scale_ori, size=(1,)), (2, 0))
        current_pos, current_ori = p.getBasePositionAndOrientation(i)
        current_ori = p.getEulerFromQuaternion(current_ori)
        print("current to future:", current_pos, current_pos + perturbation_pos)
        new_pos = current_pos + perturbation_pos
        new_ori = p.getQuaternionFromEuler(current_ori + perturbation_ori)
        p.resetBasePositionAndOrientation(i, posObj=new_pos, ornObj=new_ori)
        ...
        while True:
            ...
            if self.mode == PERTURB:
                if all([np.linalg.norm(np.array(p.getBaseVelocity(id)[0]) - np.zeros((3,))) <= self.vel_epsilon for id in self.bodies]):
                    self.perturb_time = time.time()
                    self.set_start_pos()
                    self.monte_carlo_shift(scale_pos=(0.001 * self.x_len, 0.001 * self.y_len))
                    self.mode = CHECK
...
#### python ipe3.py --save_dir /Users/kristinezheng/lookit-jenga/analysis_src/saved_results --num_trials 50 --N 10 --blocks /Users/kristinezheng/lookit-jenga/2_20_towers/7979.json###
#### python ipe3.py --display --save_dir /Users/kristinezheng/lookit-jenga/analysis_src/saved_results --num_trials 50 --N 10 --blocks /Users/kristinezheng/lookit-jenga/2_20_towers/7979.json###
### python ipe3.py --display --save_dir /Users/kristinezheng/lookit-jenga/analysis_src/saved_results --num_trials 5 --N 5 --blocks /Users/kristinezheng/lookit-jenga/2_20_towers/7979.json###